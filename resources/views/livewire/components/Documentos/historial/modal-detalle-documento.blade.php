                    ...
                    if (!empty($motivoObservacion)) {
                        if ($esObservacionRecepcionado) {
                            ...
                            if ($esPendientes) {
                                ...
                                $mostrarAlertaRoja = $estaRecepcionado;
                            }
                        }
                    }
                                    ...
                                    <div class="d-flex align-items-center">
                                        <div class="symbol symbol-25px me-2">
                                            <span class="symbol-label bg-white text-gray-800 fs-8 fw-bold border border-gray-200">
                                                {{ Str::upper(Str::substr($modeloDocumento->areaRemitente->nombre_area ?? 'N/A', 0, 1)) }}
                                            </span>
                                        </div>
                                    ...
                                    <div class="col-12" wire:key="arch-orig-{{ $archivo->id_archivo_documento }}">
                                        <div class="d-flex align-items-center bg-white border border-gray-300 rounded p-2 px-3 shadow-sm">
                                            ...
                                            <div class="d-flex flex-column flex-grow-1 overflow-hidden">
                                                <div class="fw-bold text-gray-800 text-truncate" title="{{ $archivo->nombre_original }}">{{ Str::limit($archivo->nombre_original, 35) }}</div>
                                                <div class="text-muted fs-8">{{ $archivo->tamanio_formateado ?? 'Desconocido' }}</div>
                                            </div>
                            ...
                            @if($modeloDocumento->estado)
                                @php
                                    $colorEstado = match($nombreEstadoActual) {
                                        'DERIVADO' => 'warning',
                                        'RECEPCIONADO' => 'warning',
                                        'EN TRÁMITE' => 'primary',
                                        'OBSERVADO' => 'danger',
                                        'SUBSANADO', 'RECEPCION SUBSANADA' => 'success',
                                        'ARCHIVADO' => 'primary',
                                        'ANULADO' => 'dark',
                                        'POR RECTIFICAR' => 'warning',
                                        'FINALIZADO' => 'success',
                                        default => 'info'
                                    };
                                @endphp
                                ...
                                <div class="notice d-flex bg-light-danger rounded border-start border-4 border-danger p-4 shadow-sm">
                                    <div class="d-flex flex-column w-100">
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <h5 class="text-gray-900 fw-bold m-0">
                                                <i class="ki-outline ki-information-5 fs-6 text-danger me-2"></i>
                                                Documento Observado
                                            </h5>
                                            @if($fechaObservacion)
                                                <span class="badge badge-light-danger fs-8">
                                                    {{ Carbon::parse($fechaObservacion)->format('d/m/Y') }}
                                                </span>
                                            @endif
                                        </div>
                                ...
                                <div class="notice d-flex bg-light-warning rounded border-start border-4 border-warning p-4 shadow-sm">
                                    <div class="d-flex flex-column w-100">
                                        <h5 class="text-gray-900 fw-bold mb-2">
                                            <i class="ki-outline ki-shield-tick fs-6 text-warning me-2"></i>
                                ...
                                <div class="notice d-flex bg-light-success rounded border-start border-4 border-success p-4 shadow-sm">
                                    <div class="d-flex flex-column">
                                        ...
                                        <div class="fs-7 text-gray-600">
                                            Se han atendido todas las observaciones.
                                        </div>
                                    </div>
                                </div>
                            @endif
