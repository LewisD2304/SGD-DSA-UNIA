<div> @section('breadcrumb')
    <x-breadcrumb titulo="Reporte de Usuarios">
        ...
        <x-breadcrumb.item titulo="Reporte de Usuarios" />
    </x-breadcrumb>
    @endsection
                    ...
                    <div class="row g-4 mb-7">
                        <div class="col-md-3">
                            <div class="card bg-light-primary border-0">
                                <div class="card-body d-flex align-items-center justify-content-between p-4">
                                    <div>
                                        <span class="text-muted fw-semibold d-block fs-8 mb-1">Total Documentos</span>
                                        <span class="text-gray-800 fw-bold fs-2x d-block">{{ collect($accionesPorTipo)->firstWhere('tipo', 'Documentos Creados')['cantidad'] ?? 0 }}</span>
                                    </div>
                        ...
                        <div class="col-md-3">
                            <div class="card bg-light-warning border-0">
                                <div class="card-body d-flex align-items-center justify-content-between p-4">
                                    <div>
                                        <span class="text-muted fw-semibold d-block fs-8 mb-1">Movimientos</span>
                                        <span class="text-gray-800 fw-bold fs-2x d-block">{{ collect($accionesPorTipo)->firstWhere('tipo', 'Movimientos Creados')['cantidad'] ?? 0 }}</span>
                                    </div>
                                    <i class="ki-outline ki-arrow-two-diagonals text-warning fs-3x"></i>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card bg-light-success border-0">
                                <div class="card-body d-flex align-items-center justify-content-between p-4">
                                    <div>
                                        <span class="text-muted fw-semibold d-block fs-8 mb-1">Usuarios Activos</span>
                                        <span class="text-gray-800 fw-bold fs-2x d-block">{{ $totalUsuariosActivos }}</span>
                                    </div>
                                    <i class="ki-outline ki-profile-user text-success fs-3x"></i>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="card bg-light-info border-0">
                                <div class="card-body d-flex align-items-center justify-content-between p-4">
                                    <div>
                                        <span class="text-muted fw-semibold d-block fs-8 mb-1">Total Acciones</span>
                                        <span class="text-gray-800 fw-bold fs-2x d-block">{{ collect($accionesPorTipo)->sum('cantidad') }}</span>
                                    </div>
                                    <i class="ki-outline ki-element-11 text-info fs-3x"></i>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="row g-5 g-xl-10 mb-5 mb-xl-10">
                    ...
                    <div class="d-flex align-items-center mb-6">
                        <div class="symbol symbol-40px me-4">
                            <div class="symbol-label bg-light-primary">
                                <span class="fs-7 fw-bold text-primary">{{ strtoupper(substr($usuario->nombres_persona ?? $usuario->nombre_usuario, 0, 1)) }}</span>
                            </div>
                        </div>
                        <div class="flex-grow-1">
                            @php $nombreCompleto = $usuario->nombres_persona ? $usuario->nombres_persona . ' ' . $usuario->apellido_paterno_persona : 'SISTEMA'; @endphp
                            <a href="#" class="text-gray-800 text-hover-primary fw-bold">{{ strtoupper($nombreCompleto) }}</a>
                            <span class="text-muted d-block fw-semibold fs-8">{{ $usuario->nombre_usuario }}</span>
                        </div>
                        <span class="badge badge-light-primary fw-bold">{{ $usuario->total_acciones }} acciones</span>
                    </div>
                    @empty
                    <div class="text-center py-6">
                        <p class="text-muted fs-8">No hay actividad registrada</p>
                    </div>
                    @endforelse
                </div>
            </div>
        ...
        <div class="col-md-6">
            <div class="card h-100">
                <div class="card-header">
                    <h3 class="card-title">Acciones por Tipo</h3>
                </div>
                <div class="card-body">
                    @foreach ($accionesPorTipo as $accion)
                    <div class="d-flex align-items-center mb-6">
                        <div class="symbol symbol-40px me-4">
                            <div class="symbol-label" style="background-color: {{ $accion['color'] }}20">
                                <i class="ki-outline {{ str_contains($accion['tipo'], 'Documento') ? 'ki-file' : 'ki-arrow-two-diagonals' }} fs-2" style="color: {{ $accion['color'] }}"></i>
                            </div>
                        </div>
                        <div class="flex-grow-1">
                            <span class="text-gray-800 fw-bold fs-8 d-block">{{ $accion['tipo'] }}</span>
                            <span class="text-muted fw-semibold fs-8">Registros totales</span>
                        </div>
                        <span class="badge badge-light fw-bold">{{ $accion['cantidad'] }}</span>
                    </div>
                    @endforeach
                </div>
            </div>
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title fw-bold">
                        <i class="ki-duotone ki-clock fs-2 me-2"><span class="path1"></span><span class="path2"></span></i>
