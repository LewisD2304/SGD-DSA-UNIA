...
<head>
    ...
    <style>
        :root { --kt-primary: #009ef7; --kt-primary-light: #f1faff; --kt-text-gray-800: #181c32; --kt-text-gray-600: #7e8299; --kt-text-gray-500: #a1a5b7; --kt-text-gray-400: #b5b5c3; --kt-bg-light: #f5f8fa; --kt-card-shadow: 0px 0px 20px 0px rgba(76, 87, 125, 0.02); }
        ...
        .card { border: 0; box-shadow: var(--kt-card-shadow); border-radius: 0.75rem; background-color: #ffffff; margin-bottom: 20px; }
        .card-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eff2f5; padding: 1.5rem 2rem; min-height: 70px; background: transparent; }
        .card-body { padding: 2rem; }
        .form-control-search { background-color: #f5f8fa; border: 1px solid transparent; color: var(--kt-text-gray-800); border-radius: 0.475rem; padding: 1rem 1rem 1rem 3rem; font-size: 1.1rem; font-weight: 500; transition: all 0.2s ease; }
        .form-control-search:focus { background-color: #f5f8fa; border-color: #eef3f7; color: var(--kt-text-gray-800); box-shadow: none; }
        .search-icon { position: absolute; top: 50%; left: 1.2rem; transform: translateY(-50%); color: var(--kt-text-gray-500); font-size: 1.3rem; }
        .btn-primary { background-color: var(--kt-primary); border-color: var(--kt-primary); padding: 0.8rem 1.5rem; font-weight: 600; border-radius: 0.475rem; }
        .btn-primary:hover { background-color: #0095e8; }
        .btn-warning { background-color: #ffc700; border-color: #ffc700; color: #ffffff; font-weight: 600; }
        .btn-warning:hover { background-color: #f1bc00; border-color: #f1bc00; }
        .info-box { background-color: #f9f9f9; border: 1px dashed #e4e6ef; border-radius: 0.475rem; padding: 1.25rem; height: 100%; display: flex; align-items: flex-start; gap: 1rem; transition: all 0.2s; }
        .info-box:hover { background-color: #f1faff; border-color: var(--kt-primary); }
        .symbol-circle { width: 45px; height: 45px; background-color: #fff; border-radius: 50%; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.05); flex-shrink: 0; }
        .timeline-label { position: relative; padding-left: 0; }
        .timeline-item { display: flex; flex-direction: row; position: relative; margin-bottom: 1.75rem; }
        .timeline-item:before { content: ""; position: absolute; left: 21px; top: 25px; bottom: -25px; width: 2px; background-color: #e4e6ef; z-index: 0; }
        .timeline-item:last-child:before { display: none; }
        .timeline-item:last-child { margin-bottom: 0; }
        .timeline-badge { flex-shrink: 0; background-color: #ffffff; width: 44px; height: 44px; border-radius: 50%; border: 2px solid #e4e6ef; display: flex; align-items: center; justify-content: center; z-index: 1; position: relative; margin-right: 1.25rem; }
        .timeline-badge i { font-size: 1.2rem; color: var(--kt-text-gray-500); }
        .timeline-badge.success { border-color: #50cd89; } .timeline-badge.success i { color: #50cd89; }
        .timeline-badge.primary { border-color: var(--kt-primary); } .timeline-badge.primary i { color: var(--kt-primary); }
        .timeline-badge.info { border-color: #7239ea; } .timeline-badge.info i { color: #7239ea; }
        .timeline-badge.warning { border-color: #ffc700; } .timeline-badge.warning i { color: #ffc700; }
        .timeline-badge.danger { border-color: #f1416c; } .timeline-badge.danger i { color: #f1416c; }
        .timeline-content { flex-grow: 1; background-color: #f9f9f9; padding: 1rem; border-radius: 0.475rem; }
        .badge { padding: 0.5rem 0.6rem; border-radius: 0.325rem; font-weight: 600; font-size: 0.75rem; }
        ...
        @keyframes placeholder-glow { 50% { opacity: .5; } }
    </style>
</head>
<body>
    ...
    <div class="container mb-5">
        ...
        <div class="card border-0 mb-8">
            <div class="card-body py-10">
                ...
                <form method="post" action="{{ route('consulta.buscar') }}" class="mx-auto mw-700px position-relative" id="formConsulta">
                    @csrf
                    ...
                    <div class="input-group">
                        ...
                        <button type="submit" class="btn btn-primary" id="btnBuscar">
                            ...
                            <span id="btnSpinner" class="d-none">
                                <span class="spinner-border spinner-border-sm align-middle ms-2"></span> Buscando...
                            </span>
                        </button>
                    </div>
                    @error('expediente')
        ...
        <div id="loadingSkeleton" class="w-100 mt-5 placeholder-glow d-none">
            <div class="row g-6">
                <div class="col-lg-7">
                    <div class="card h-100">
                        <div class="card-header border-0 pb-0">
                            <div class="card-title m-0 d-flex flex-column w-100">
                                <span class="placeholder col-6 mb-2" style="background-color: #e4e6ef; height: 1.5rem; border-radius: 0.42rem;"></span>
                                <span class="placeholder col-4" style="background-color: #e4e6ef; height: 1rem; border-radius: 0.42rem;"></span>
                            </div>
                        </div>
                        <div class="card-body pt-6">
                            <div class="d-flex flex-wrap gap-5 mb-8">
                                <span class="placeholder col-5" style="background-color: #e4e6ef; height: 4rem; border-radius: 0.42rem;"></span>
                                <span class="placeholder col-5" style="background-color: #e4e6ef; height: 4rem; border-radius: 0.42rem;"></span>
                            </div>
                            <div class="mb-8">
                                <span class="placeholder col-12 mb-2" style="background-color: #e4e6ef; height: 1rem; border-radius: 0.42rem;"></span>
                                <span class="placeholder col-10" style="background-color: #e4e6ef; height: 1rem; border-radius: 0.42rem;"></span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-5">
                    <div class="card h-100">
                        <div class="card-header border-0 pb-0">
                             <span class="placeholder col-6" style="background-color: #e4e6ef; height: 1.5rem; border-radius: 0.42rem;"></span>
                        </div>
                        <div class="card-body pt-6 d-flex flex-column gap-4">
                            <div class="d-flex align-items-center">
                                <span class="placeholder rounded-circle me-3" style="width: 40px; height: 40px; background-color: #e4e6ef;"></span>
                                <span class="placeholder col-8" style="background-color: #e4e6ef; height: 1rem;"></span>
                            </div>
                            <div class="d-flex align-items-center">
                                <span class="placeholder rounded-circle me-3" style="width: 40px; height: 40px; background-color: #e4e6ef;"></span>
                                <span class="placeholder col-6" style="background-color: #e4e6ef; height: 1rem;"></span>
                            </div>
                             <div class="d-flex align-items-center">
                                <span class="placeholder rounded-circle me-3" style="width: 40px; height: 40px; background-color: #e4e6ef;"></span>
                                <span class="placeholder col-7" style="background-color: #e4e6ef; height: 1rem;"></span>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="timeline-label">
                                ...
                                @php
                                    ...
                                    $colorClass = match($movEstado) {
                                        ...
                                        'DERIVADO' => 'primary',
                                        'ARCHIVADO' => 'info',
                                        'OBSERVADO', 'RECHAZAR RECTIFICACION' => 'danger',
                                        'POR RECTIFICAR', 'SOLICITAR RECTIFICACION' => 'warning',
                                        default => 'secondary'
                                    };
                                    $iconClass = match($movEstado) {
                                        ...
                                        'SOLICITAR RECTIFICACION' => 'bi-file-earmark-text',
                                        'RECHAZAR RECTIFICACION' => 'bi-x-circle',
                                    ...
                                    <div class="timeline-content">
                                        <div class="d-flex flex-stack mb-1">
                                            @if($mov->observacion_doc_movimiento)
                                                <span class="text-gray-800 fw-bold fs-6 cursor-pointer text-decoration-underline"
            ...
            <div class="card border-dashed border-warning bg-light-warning">
                <div class="card-body text-center py-15">
                    ...
                    <p class="text-gray-600 fs-5">
                        El número <strong>"{{ $expediente ?? '' }}"</strong> no existe en nuestros registros.<br>
                    ...
                    <form method="post" action="{{ route('consulta.rectificar') }}" enctype="multipart/form-data">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="file" name="archivos_evidencia[]" id="archivos-evidencia-input" class="form-control" multiple accept=".pdf,.png,.jpg,.jpeg" style="display: none;">
    ...
    <script>
        // Lógica para mostrar SKELETON (Efecto de carga) al enviar formulario
        ...
        // Lógica del modal de detalle de movimiento
        ...
        // (Aquí puedes pegar el JS de la vista previa de archivos si lo necesitas, es el mismo que tenías)
        // ...
    </script>
    ...
    <script>
        Swal.fire({
            ...
            text: "{{ session('status') }}", // Corregido: Sin espacios extra
    ...
    <script>
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: "{{ $errors->first('motivo') }}", // Corregido: Sin espacios extra
            confirmButtonText: 'Aceptar',
